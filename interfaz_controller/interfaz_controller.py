...
def param_stoiip(df, row, dist_col, loc_col, scale_col, iter, sc_col=None, lim_min_col=None, lim_max_col=None):
    """
    This function has as a goal, return numpy arrays of any stoiip variable like area, thickness, porosity, oil
    saturation and Boi. Furthermore, this numpy array contains random variables from various continuous distributions.
    ...
    if df.loc[row, dist_col] == 'Log-Normal' or df.loc[row, dist_col] == 'Triangular':
        if df.loc[row, dist_col] == 'Log-Normal':
            param = lognorm.rvs(s=df.loc[row, sc_col], loc=df.loc[row, loc_col], scale=df.loc[row, scale_col],
                                size=iter)
            ...
            param = triang.rvs(c=df.loc[row, sc_col], loc=df.loc[row, loc_col], scale=df.loc[row, scale_col],
                               size=iter)
